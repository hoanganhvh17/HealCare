/* =========================================================================
   ...
   Bản port của staff-notifications.js, đổi endpoint sang /api/notifications và
   đổi id sang userNotif* để hai chuông không đụng nhau — trang
   user/medical-record-detail.html nhúng CẢ header bệnh nhân lẫn (với bác sĩ/admin)
   footer khu quản trị, nên trùng id là chuyện có thật chứ không phải giả định.

   Markup nằm trong fragment header dùng chung (user/include/header :: header-nav),
   nên script này phải tự chịu được việc không có chuông trên trang (khách chưa
   đăng nhập không được render chuông).
...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!list) {
            return; // trang này không có chuông (khách vãng lai)
        }

        var badge = document.getElementById('userNotifBadge');
        var toggleBtn = document.getElementById('userNotifToggle');
