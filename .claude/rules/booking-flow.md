...
Slot buttons are also hardcoded in `user/appointment.html`, `user/booking-edit.html` and `receptionist/walk-in-form.html`, and an `allTimeSlots` JS array is duplicated in `user/doctors.html` and `user/index.html` — all five must match the Java lists too. `user/doctor-details.html` **no longer declares one**: it reads the whole week from `GET /api/chat/doctor-availability?detail=slots`, so its slot labels and their morning/afternoon split both come from the server. The `/skills/sync-slot-grid` skill holds the full checklist (10 places including `LeavePolicy.OFFICE_START`/`OFFICE_END`, the AI prompt and `normalizeTimeHint`); use it rather than re-deriving the list.
...
- **PAY_AT_COUNTER** — reserve and stop. The booking stays `PENDING` + `UNPAID`; the patient pays at the desk on arrival. Added 2026-08-19 so people can be given the system to try without touching real money.

Three rules hold that fourth branch together, and each one is a trap that fails silently:

- **`BookingCleanupTask` must exclude it, and the query must name NULL explicitly.** That job cancels every `PENDING` + `UNPAID` booking older than 3 minutes — exactly the shape of a pay-at-counter booking, so without the exclusion the feature dies three minutes after it ships and the patient just sees the appointment vanish. The finder is now `findAbandonedPrepayBookings`, whose `WHERE` carries `(b.paymentMethod IS NULL OR b.paymentMethod <> :keepMethod)`. **The `IS NULL` half is load-bearing:** `payment_method` is nullable and legacy rows hold NULL, and in SQL `NULL <> 'X'` is UNKNOWN rather than TRUE — drop it and every old booking escapes the job forever, holding its slot with nothing in any log. It is deliberately a **deny-list**, which preserves today's behaviour for unknown values at the price that **any payment method added later is cancelled after 3 minutes** unless someone remembers this line. That is the twin of the `else` trap below.
...
**`whyCannotBookWithoutPayment(user)` is not a violation of that rule — it is the other side of it.** The rule rejects guarding *slot contention*, which is a race. How many unpaid bookings **this account** is holding is a stable property of the user: only their own actions change it, and one `COUNT` answers it **before the radio is even drawn**, so the form can disable the option and print the reason. The name deliberately avoids the word `Reserve` so nobody is tempted to fold slot races back in later. Limit is `MAX_PAY_AT_COUNTER_BOOKINGS = 2`, counted over `paymentMethod = PAY_AT_COUNTER` + `status IN (PENDING, CONFIRMED)` + `appointmentDate >= today`.

Four things about that count, each of which was a decision rather than an accident:

- **It lives in `processAppointment`, never in `reserve()`.** `reserve()` is the shared funnel for the receptionist walk-in desk and for rescheduling too; a per-user quota there would refuse to register the patient standing at the counter. Same reasoning that keeps the "slot must still be in the future" rule in the controller.
- **It counts only pay-at-counter bookings, not every upcoming one.** Booking-on-behalf is a supported feature (`patientName` / `patientPhone`), so counting all upcoming bookings would stop one account booking for three family members even after paying three times. Prepaid bookings already carry their own entry fee against spam.
- **`appointmentDate >= today` is what makes the quota release itself.** There is no no-show screen anywhere in the app, so without that bound a patient who forgets twice is locked out of booking **permanently** and no member of staff has a button to fix it.
- **The check-then-insert race is accepted.** Two simultaneous POSTs can both pass the `COUNT` and produce a third booking. This is a speed bump against spam, not a money rule — the same position the project already takes with the in-process `ReentrantLock`.
