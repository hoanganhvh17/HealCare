...
`reasoning`, `ai_reply`, `speech_reply`, `suggested_prompts` (always 3), `recommended_departments` (array of department IDs), `is_emergency`, `patient_summary`, `booking_intent`, `booking_target`, `lookup`.

`lookup` is the **routing field for questions**, added so one key can carry every future lookup rather than migrating the schema again per feature: `{type: none|doctor_schedule|my_bookings|doctor_info|doctor_filter, doctor_name, date, session, scope, filter{gender, sort_by}}`. Prompt section **5D** defines the five types; a bullet in **5B** states that `lookup` is routing only and does **not** license the model to answer about availability itself — without it the key reads as permission and reverses the very rule it exists to work around. `MAX_TOKENS` went 1200 → 1500 with it, since a truncated JSON prints raw into the chat bubble.
...
`speech_reply` is the spoken-aloud variant of `ai_reply` (≤2 sentences, no emoji/HTML/disclaimer) used by the voice layer. The prompt states the key count in **two places, worded differently** — "ĐÚNG 10 KEYS" and "đủ 10 trường" (both in `AiService.PATIENT_BASE_PROMPT`) — so grepping for one misses the other; keep both in step when adding or removing a key. The voice layer always falls back to `MediTrustVoice.toSpeechText(ai_reply)` when the model omits it, so a non-compliant model degrades rather than breaks.
...
- Tuning constants: `DEFAULT_RATE = 1.5` and `MAX_CHUNK_CHARS = 300` in `meditrust-voice.js` (speaking speed and how much text goes into one utterance — both one place for the whole app), and `SILENCE_MS = 1100` in `meditrust-voice-call.js` (how long a pause means "done talking"). `joinSentences()` in the same file removes the long stop the voice makes at every full stop; see [coding-conventions.md](coding-conventions.md) for the three things it must not touch.
...
- **`/api/chat/doctors/department/{id}` ranks by real availability, and the browser obeys that order.** It takes optional `date` / `time` / `session` and returns doctors sorted by: **pinned `doctorId` → free at the requested slot/session → fewest `nearbyLoad` (bookings within ±90 min) → `dayLoad` → experience desc → id**. The truncation to 3 happens **after** the sort.

  This exists because the chat had **no ranking at all** — `grep '\.sort('` in `ai-chat.js` returned nothing. The endpoint was time-blind, `findByDepartmentId` has no `ORDER BY`, and `.limit(3)` ran *before* availability was computed, so with ~6 doctors per department **half the department was never evaluated** and `ai-chat.js` took `availableDoctors[0]` — the same human every session, regardless of the hour the patient asked for.

  Three rules hold it together:
  - **Ranking reorders; it never removes.** Filtering out busy doctors breaks two things at once: a doctor the patient named by name vanishes → `doctorNotFound` → *"Em chưa tìm thấy bác sĩ X"* about a doctor who exists; and a fully-booked department returns `[]` → `NO_DOCTORS`, killing the whole `/slot-alternatives` reason path.
  - **The pinned `doctorId` is first unconditionally**, including when that doctor is busy — same invariant as before, now expressed as the first sort key.
  - **The scan is day-major**: one day at a time across *every* doctor, stopping at the first day anyone is free. The old per-doctor loop stopped at each doctor's own first open day, so one preview could be tomorrow and another next Friday — the two were never comparable.

  An off-grid `time` here is silently ignored rather than answered with `OUTSIDE_HOURS`; that sentence belongs to `/slot-alternatives`, and two endpoints answering the same question is how the slot grid grew copies before.
- **Both callers must send the same wishes.** `parseWishes(userText, bookingTarget)` and `buildDeptUrl(...)` in `ai-chat.js` are the single place the patient's time/session/date is derived and attached. The doctor carousel and `resolveBookingHandoff` both go through them — when only one did, the patient read three cards and the confirmation card underneath booked a fourth doctor. `buildDeptUrl` deliberately **omits `date` unless the patient actually named one**: otherwise the date would have to come from the very response it is requesting.
...
- **`reason` + `reasonText` say WHY**, and are the point of the endpoint. `reason` is one of `OFF_DUTY` / `BOOKED` / `BLOCKED` / `HELD` / `PAST` / `TOO_FAR` / `OUTSIDE_HOURS` / `FREE`; `reasonText` is the ready-made Vietnamese sentence used by **both** the chat card and the spoken line.
  ...
  **The wording of `OFF_DUTY` and `BOOKED` is load-bearing and must stay opposed.** `OFF_DUTY` used to read *"T7 09/08 bác sĩ X không có ca khám ạ."* — which in Vietnamese also parses as "nobody has booked him", i.e. the exact opposite of what it means. Patients read it as "the doctor is free", answered "so why won't you book me?", and were then moved to another day with no idea why. Every sentence now names its cause unambiguously: `OFF_DUTY` always says **"không đăng ký ca làm việc"** (the doctor's own registration), `BOOKED` always opens with **"bác sĩ CÓ khám … nhưng"** (the doctor works, the slot is taken). Never reintroduce the phrase "không có ca khám".

  `BLOCKED` distinguishes "báo bận cả ngày" from "báo bận buổi …" via `DoctorBlockTime.startTime/endTime`. It deliberately does **not** read `leaveRequestId` and must **never** print `DoctorBlockTime.reason` — that field carries the leave type, which is staff data.
...
- **`resolveCanonicalSlot` maps a time to the slot that CONTAINS it**, not `startsWith`. "9 giờ 15" resolved to nothing and the assistant answered "phòng khám chỉ nhận đặt khám trong giờ hành chính…" about a time squarely inside office hours; it now becomes `09:00 - 09:30`. Only genuinely off-grid times (12:15, 19:00) still return `OUTSIDE_HOURS`. `reasonText` is written in the **machine formats** `T3 28/07` and `13:30 - 17:30` so `MediTrustVoice.humanizeSchedule()` speaks it correctly; do not add a separate speech variant. Without this the assistant could only ever say "đã kín lịch", which was simply wrong when the doctor was off duty.
...
- **`DaySlots.summarizeReasonsIn` replaced `worstReasonIn`, which contradicted `blockReason`'s own javadoc.** The old function picked one winner with the preference `BOOKED > BLOCKED > HELD > OFF_DUTY > PAST`, the exact reverse of the order `blockReason` argues for — so an afternoon that was 6 slots off-duty and 2 slots booked reported only "đã có người đặt", and the patient again concluded the doctor was working. It now returns `ReasonSummary(dominant, counts, free, total)`: `dominant` is the highest count, ties broken by `OFF_DUTY, BLOCKED, BOOKED, HELD, PAST` (facts about the **person** outrank facts about **one slot**, which can change in a minute), and `reasonText` names **both** reasons when two are present, capped at two clauses because the voice layer reads it verbatim. `PAST` is excluded from the runner-up slot — "đã trôi qua" is a property of the clock, not the doctor, and mentioning it implies those slots were otherwise bookable. Payload gains `reasonBreakdown` and `freeCountInRange`.
- `requestedDoctorWorkingRanges` is the complement of the off-duty set, adjacent slots merged (the lunch break splits it naturally). **Display only — never invert it into a whitelist:** a doctor with no `Schedule` rows has an empty off-duty set, which would turn "unrestricted" into "nothing allowed".
  ...
  **It must be read together with `scheduleKnown`**, because empty has two opposite meanings. `BookingService.hasRegisteredSchedule(doctorId, date)` (new, `findEffective(...).isEmpty()` inverted) supplies it. With `scheduleKnown = false` the merge loop would emit `07:30 - 11:30` + `13:30 - 17:30` — a completely fabricated working day for someone who never registered — so `workingRanges()` now short-circuits to empty and the sentence says "hệ thống chưa có lịch đăng ký" instead. **This is an explanation-layer flag only: `slotsOutsideWorkingHours` / `isSlotWithinWorkingHours` keep the "empty = unrestricted" rule**, or the 132 seeded doctors become unbookable.
- **`otherTimes` may be on a different day.** When the doctor is off for the whole requested day, the scan moves to their nearest working day within 7 days, so every entry carries its own `date` and `slotLabel`. Every consumer must use `item.date`, not the handoff's date — otherwise the button books the right time on the wrong day. Only one day is ever emitted, so `distance` (minutes) stays meaningful.
  ...
  **The day change must appear in the HEADLINE, not a sub-line.** The server sets `otherTimesDate` / `otherTimesMovedDay` / `otherTimesText` ("Ngày làm việc gần nhất của bác sĩ X là T3 12/08 ạ."); `buildSlotFullHtml` appends it to the bold first line and `describeSlotFull` speaks it right after the reason, before either direction. Previously the only hint was the phrase "ngày bác sĩ có ca khám gần nhất" buried in a sub-line, so a patient read "dời sang khung giờ gần nhất", pressed the button, and landed on the wrong day.
- `sameTimeDoctors` is ranked by `nearbyLoad` (that doctor's bookings within `NEARBY_MINUTES` of the requested time), then `dayLoad`. Fewest first, so the patient waits least at the clinic. Both surfaces must state the **reason** out loud ("em gợi ý anh/chị bác sĩ này vì quanh giờ đó chỉ có 1 ca…") — a bare name gives the patient nothing to choose on.
  ...
  **It is rendered on the happy path too, not only on failure.** The server returns this ranked list even when the requested doctor *is* free, and the browser used to throw it away — so a patient was handed one name and never learned that two colleagues were free at the same hour. `finishBookingHandoff` now prints them with an **"Đổi"** button, built by the same `buildDoctorChoiceRow` as the fallback card's "Chọn". Three constraints: `pickOtherDoctors` **must filter the chosen doctor out** (the server's list includes them, so without it the card offers a swap to the doctor it just picked); the count comes from `others.length`, never a hard-coded 2; and an empty list prints **nothing**, because "trong khoa chỉ còn mỗi bác sĩ X" is a fresh negative claim nobody verified.

  **The non-fallback path must still leave `pendingAlternatives = null`.** Attaching `alternatives` to the handoff does not arm it — `resolveAlternativeChoice` keys off `pendingAlternatives`, which is assigned in exactly one place, under `fallback`. But if it were ever set here, a bare "vâng" on the next turn would map to `directions[0]` and fire a **second `/hold-slot` and a second countdown** on a different doctor, and `buildAlternativeContext` would order the model to pick a direction for a patient who already has a confirmed slot.
...
`selectedDoctor.availableSlots` is only a **4-slot preview of the nearest day** (`AiController` stops at the first day with any opening). It is decoration for the doctor cards and is never the basis of a decision. The **other** DTO fields — `matchedSlot`, `matchedSlotLabel`, `matchedDate`, `nearbyLoad`, `dayLoad`, `matchesRequest` — are the opposite: facts the server just verified, which the card *is* allowed to quote. Branch A prefers `selectedDoctor.matchedDate` over re-parsing the preview label, whose year is a guess.

**Every superlative must name its scope.** The confirmation card used to read *"Em xin phép chọn giúp anh/chị khung giờ trống sớm nhất"* on every `suggested` handoff, with zero data about any other doctor — while the doctor itself was `availableDoctors[0]`. It now either states the verified reason (*"em chọn giúp anh/chị bác sĩ X vì quanh giờ đó bác sĩ chỉ có 1 ca"*) or, when the patient named no time, keeps the superlative but scopes it: *"khung trống sớm nhất **của bác sĩ**"*. **No numbers → no reason sentence**; `pickNearbyLoad`/`pickDayLoad` are `null` when the server returned nothing, and `null` means stay quiet, not "0 ca". `describeLoad` ("quanh giờ đó") may only be used when the patient actually named a time or session — otherwise `describeDayLoad` ("hôm đó"), since "quanh giờ đó" would point at a moment the patient never mentioned. Both have a twin in `meditrust-voice-call.js`; reword one and the card and the spoken line disagree.

The function has exactly three branches, and only confirms `fallback: false` when what the patient asked for was actually honoured:
...
- `extractDateHint` resolves weekday names (`thứ ba` / `thứ 3` / `t3`, `chủ nhật` / `cn`), `tuần sau`, `ngày mốt`, `cuối tuần này`, `N ngày nữa`, `mùng 5 tháng 8`, and the **session-qualified forms of today** — `hôm nay` / `sáng nay` / `chiều nay` / `trưa nay` / `tối nay`. Without those last four, "bác sĩ X **chiều nay** bận à?" carried no date at all, so the handoff borrowed `lastHandoffDate` and answered about a day the patient never asked about. They sit **after** the weekday loop for the reason already documented for `hôm nay`, and deliberately **skip `sessionAlreadyPassed`**: a patient asking at 18h about "chiều nay" means the afternoon that just passed, and rolling to next week answers a different question — the server's `PAST` code is the right answer there. The two-word `sang nay` / `toi nay` are safe; only the **bare** `sang` / `toi` are banned (see `extractSessionHint`). When the named weekday **is today**, it stays today unless that session has already elapsed (sáng after 11:30, chiều after 17:30, and **no session named → 17:30**, so "đặt lịch thứ 5" at 18h on a Thursday means next Thursday), in which case it rolls forward a week.
...
## Khách HỎI về lịch, không phải xin đặt lịch

Until this existed, **the system could only look up a schedule while the patient was BOOKING, never while they were ASKING.** `/api/chat/slot-alternatives` had exactly one caller — `fetchSlotAlternatives`, inside `resolveBookingHandoff` — and that function returns `null` the moment `booking_intent` is false. Meanwhile prompt section 1 and 5B **forbid** the model from asserting whether a doctor works on a given day, and 5B promises "hệ thống lo phần lịch". For a plain question the system printed nothing at all, so "bác sĩ X chiều nay bận à?" got a content-free reply the patient read as "he's free".

**`GET /api/chat/doctor-availability?doctorId=&date=&days=&session=&sessionId=`** is the read-only answer path. It is a **separate endpoint, not a reuse of `/slot-alternatives`**, for three structural reasons: that one requires `departmentId` and loops the whole department; it is semantically an *offer* (`sameTimeDoctors`, `otherTimes`, `slot`) whose shape invites the caller to feed it into `pendingAlternatives`; and it is single-date by construction, so it cannot answer "tuần này bác sĩ làm ngày nào". Returns `anchor{dayLabel, scheduleKnown, workingRanges, dayState, reason, reasonText, reasonBreakdown, freeCount, firstFreeSlot}`, a `week[]` of the same shape, and `summaryText`. `dayState` ∈ `NO_SCHEDULE | OFF_ALL_DAY | FULL | PARTIAL | PAST`.
...
### Detection: `lookup` key + regex, and here the regex outranks the model
`resolveLookup(aiData, userText)` in `ai-chat.js` is the single entry point, routing to availability / my-bookings / doctor-info / doctor-filter. Each has a `looksLike…Question()` gate built on the space-padded idiom, plus `aiData.lookup.type` from the model.

**This inverts the house doctrine that `booking_intent === true` is the primary path**, and deliberately: the model is *prompt-forbidden* from reasoning about availability, so its signal on this topic is the weaker one. `looksLikeAvailabilityQuestion` has four exclusion gates, and the first is the important one — **any sentence containing `đặt` / `book` / `lấy lịch` returns false**, so "bác sĩ X chiều nay còn chỗ không, đặt cho em" keeps the booking path. The third gate rejects `(tôi|em|mình…) + (bận|rảnh|nghỉ)`: "chiều nay **tôi** bận" is the *patient* being busy, the same trap already documented for "sáng" in `normalizeTimeHint`.

`resolveBookingHandoff` gained a matching early-out at its top, because **`booking_intent` is sticky** — prompt section 5 tells the model to keep fields the patient didn't mention, so mid-booking a question still arrives with `booking_intent: true` and used to take a 3-minute soft lock and start a redirect countdown *for a question*.

### Four things the lookup branch must never do
It must not set `pendingAlternatives`, must not `POST /api/chat/hold-slot`, must not call `startRedirectCountdown`, and must not write `lastHandoffDate`. The first three hold by construction as long as it **never calls `finishBookingHandoff`** — that is the only place any of them live. The fourth needs its own discipline: the branch remembers the doctor in a separate `lastAvailabilityDoctor` (so "thế mai thì sao?" works) precisely because writing the asked-about date into `lastHandoffDate` would make the next correction turn borrow a date the patient only enquired about.

Cards render through `buildAvailabilityHtml` and friends in **info blue (`#e0f2fe`)**, deliberately not the amber of `buildSlotFullHtml` — amber means "your request failed", and a question did not fail. Bookable days carry a plain link; nothing auto-navigates. The voice counterpart is `describeAvailability` in `meditrust-voice-call.js`, which **must set `awaitingConfirm = null` and end on an open question**: there is no appointment for a "vâng" to confirm. Ship it with the chat branch — without it the voice layer falls through to `say(spoken)` and the schedule answer is never spoken.

### Every lookup needs a voice branch, and the payload must carry it
`notifyReply` passes **`lookup`** (the whole `resolveLookup` result) alongside `availability`. It carried only `availability` at first, so the other three kinds rendered a card and **said nothing at all** — the one mode where the patient is not looking at the screen. `meditrust-voice-call.js` has a branch per kind: `describeDoctorFilter`, `describeDoctorProfile`, `describeMyBookings`. All three follow the availability rules (`awaitingConfirm = null`, end on an open question) plus three of their own:

- **Every spoken answer must fit ONE utterance.** `SPEECH_BUDGET = 270` (under `MAX_CHUNK_CHARS = 300`); `describeDoctorFilter` reads at most 3 doctors and **drops the last one until the sentence fits**. Over the budget, `splitIntoChunks` cuts at a full stop and the browser inserts a real gap between utterances — the "assistant freezing mid-list" bug in a new costume.
- **The list kept for selection is exactly the list read aloud.** `describeDoctorFilter` returns `{text, offered}` and the branch assigns `pendingChoiceList = offered`. Keeping more would let "người thứ ba" pick a name the patient never heard.
- **One criterion, one unit, every row.** Under `sortBy=rating` every doctor is read as stars-and-count (or "chưa có lượt đánh giá nào"); under `experience`, years; under `price`, price. Mixing "5 sao" and "18 năm kinh nghiệm" in one list gives the listener nothing to compare — and unlike a card, speech cannot be re-scanned.

`pendingChoiceList` is the voice twin of `pendingAlternatives`: `resolveDoctorChoice` answers **locally, before any model call** (the model never saw the list), matching a name first and only then an ordinal — "bác sĩ Hai" is a person, not "người thứ hai". No match returns `null` and the turn goes to the model; guessing would open the booking page for a doctor the patient never chose. It is cleared at the top of every `onChatReply`, and in `startCall` / `hangUp` / `enterEmergency`, so a stale list cannot outlive its question or its call.

### Picking a doctor must immediately produce a slot — and the doctor must stick
Two rules, and the second one is the load-bearing half.

**`offerNearestSlot` runs the moment the patient picks.** It calls `/api/chat/doctor-availability?doctorId=&days=7`, takes the first `week[]` entry with a `firstFreeSlot`, and offers it. When that day is **not** the first day of the scan, it prefixes `anchor.reasonText` — so the patient hears *why* the nearest slot is on another day ("không đăng ký ca làm việc" vs "đã có người đặt") instead of an unexplained date. If nothing is free in 7 days it **asks** whether to look at another doctor and leaves `awaitingConfirm = null`; it never substitutes one. `confirmBooking` then opens `/appointment` prefilled with doctor + date + slot, so voice still never creates a booking.

Before this, the branch only said "em mở trang đặt lịch để anh/chị chọn khung giờ nhé?", which pushed the patient into a follow-up turn — and that turn is where the real damage happened:

**`lastChosenDoctor` pins the patient's choice across turns.** A follow-up like "chọn luôn giờ cho tôi chiều nay" names no doctor, and the model routinely drops the name from `booking_target`. `resolveBookingHandoff` then had nothing to pin, fell through to `availableDoctors[0]`, and **silently booked a different doctor** — right after the patient had compared ratings to choose one. Verified: without the pin the same sentence resolves to a different name; with it, the chosen doctor is kept and the request comes back `fallback` + `reason: OFF_DUTY` with the explaining sentence.

- Written in exactly one place, `finishBookingHandoff`, so every confirmed card (normal flow, alternative choice) updates it; the voice layer has no card, so it calls `window.MediTrustChat.rememberChosenDoctor(doc)`.
- **Only pinned when the department matches** (or none is known) — a patient describing new symptoms in another department must not stay stuck on the old doctor.
- An explicitly named doctor still wins, and `parseCancelIntent` clears it.
- The "never silently substitute" guard was widened to cover it: `!selectedDoctor && (requestedDoctorName || candidateDoctorName)` returns `doctorNotFound` rather than falling back to `availableDoctors[0]`.
...
- **`GET /api/chat/my-bookings`** — the patient's own appointments. **Declared `authenticated()` in block 0 of `SecurityConfig`**, above the `/api/chat/**` permitAll; without that line it hands any stranger a patient's schedule. Each row carries `cancelBlockReason` / `rescheduleBlockReason` from `BookingService.whyCannotCancel` / `whyCannotReschedule`, so the chat card hides a button exactly when the server would refuse it.
...
- Keys: `hasReview desc → avgRating desc → reviewCount desc → experience desc → id`. The third key is why 5.0-from-20-reviews outranks 5.0-from-one.
- **Unrated doctors are ranked last, never removed.** The whole hospital has only a few dozen reviews; dropping them returns `[]` and answers a perfectly reasonable question with "em chưa tìm được bác sĩ nào". The card prints "Các bác sĩ dưới đây chưa có lượt đánh giá nào ạ:" at the boundary, and says so outright when *nothing* is rated.
- Every row prints the **count** next to the score (`⭐ 4.8 (12 đánh giá)`) — the count is the only thing that distinguishes a real score from the advertised default.
- Ratings for the whole candidate set come from **`ReviewService.getRatingStats(ids)`, one aggregate query**. Ranking has to score every doctor *before* truncating, and per-doctor `getAverageRating` + `countReviews` would be 2 × 132 queries when no department is given. A doctor with no reviews is **absent from the returned map** rather than present with `0.0` — never `COALESCE` that away, it is the distinction the whole feature rests on.

### A superlative outranks the "đặt lịch" rule
`looksLikeDoctorFilterQuestion` normally yields to the booking path (`looksLikeBookingRequest` → false), like every other lookup. `DOCTOR_SUPERLATIVE` is the **one exception**: "đặt lịch với bác sĩ đánh giá tốt nhất" routes here anyway, and `resolveBookingHandoff` has a matching early-out beside the availability one.

The reason is that the booking path has *nothing to resolve* — no doctor name, no time, usually no department — so it either dies at `NO_DEPARTMENT` or takes `availableDoctors[0]`, i.e. it picks for the patient the very thing they asked to have compared. The filter card's "Chọn" button opens `/appointment`, so booking still happens, one deliberate step later. Without the early-out both branches run: the comparison table prints *and* a 3-minute soft lock plus a redirect countdown fire for one row of it.

The superlative regex **requires the word "nhất"** — that is what keeps "bác sĩ Bình đánh giá mấy sao?" (one person → `doctor_info`) apart from "bác sĩ nào đánh giá tốt nhất?" (rank the department). `NOT_A_DOCTOR_NAME` also gained "đánh giá", "uy tín", "kinh nghiệm" and friends, or `extractDoctorName` reads the criterion as a person and the patient is told *"Em chưa tìm thấy bác sĩ có đánh giá tốt nhất"*.
...
- **Each allergy carries its `source` into the prompt** ("nguồn: bệnh nhân tự khai" / "bác sĩ ghi nhận") and the prompt requires the model to flag patient-declared entries as unconfirmed. A doctor acting on a warning needs to know whether it came from a colleague or from what the patient remembered. See [medical-records.md](medical-records.md) for where those rows come from.
