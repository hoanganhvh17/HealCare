...
- `/doctor/risk-assessment/**` (dự đoán nguy cơ bệnh) rơi vào `/doctor/**` sẵn có nên **không cần matcher mới** — đúng lý do `DoctorExamAiController` chọn tiền tố `/api/doctor/chat/`. Sidecar Python đứng sau nó **không** có endpoint nào lộ qua Spring: nó nghe `127.0.0.1:8001`, xác thực bằng bí mật ở header `X-ML-Secret` (so bằng `MessageDigest.isEqual`/`hmac.compare_digest`, để rỗng là đóng hẳn), và **tuyệt đối không được thêm `location` nào trong nginx trỏ tới cổng đó**.
